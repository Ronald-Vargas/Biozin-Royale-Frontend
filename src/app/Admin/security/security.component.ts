...
@Component({
  standalone: true,
  imports: [
    CommonModule, AtmosphereComponent, SvgIconComponent, AdminHeaderComponent, AdminNavComponent,
...
export class AdminSecurityComponent implements OnInit {
  ...
  changePass() { if (this.hasPassword) this.router.navigate(['/admin/seguridad/cambiar-contrasena']); }
  ...
  get visibleSessions(): SessionRow[] {
    return this.sessions.slice(0, AdminSecurityComponent.MAX_VISIBLE_SESSIONS);
  }
  ...
  goSesiones(): void {
    this.router.navigate(['/admin/seguridad/sesiones']);
  }
  ...
  get visibleHistory(): HistEntry[] {
    return this.history.slice(0, AdminSecurityComponent.MAX_VISIBLE_HISTORY);
  }
  ...
  goHistorial(): void {
    this.router.navigate(['/admin/seguridad/historial']);
  }
